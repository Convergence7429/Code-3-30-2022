...
public class Shooter {
    ...
    //             + (distanceFromTapeToCenterOfHub + 24.0f) - horizontalDistanceFromLimelightToShooter);
    // }
    ...
    public double dumpShotSpeed = 0.4325;
    ...
    public boolean centerRobotOnTarget() {
        ...
        if (xAngle < -15.0) {
            Robot.drive.driveTrainByControls(Constants.stick.getRawAxis(1), Constants.stick.getRawAxis(0), -0.20, true);
            return false;
        } else if (xAngle > 15.0) {
            Robot.drive.driveTrainByControls(Constants.stick.getRawAxis(1), Constants.stick.getRawAxis(0), 0.20, true);
            return false;
        } else if (xAngle < -4.0) {
            Robot.drive.driveTrainByControls(Constants.stick.getRawAxis(1), Constants.stick.getRawAxis(0), -0.12, true);
            return false;
        } else if (xAngle > 4.0) {
            Robot.drive.driveTrainByControls(Constants.stick.getRawAxis(1), Constants.stick.getRawAxis(0), 0.12, true);
            return false;
        } else if (xAngle < -1.0) {
            Robot.drive.driveTrainByControls(Constants.stick.getRawAxis(1), Constants.stick.getRawAxis(0), -0.06, true);
            return false;
        } else if (xAngle > 1.0) {
            Robot.drive.driveTrainByControls(Constants.stick.getRawAxis(1), Constants.stick.getRawAxis(0), 0.06, true);
            return false;
        } else {
            Robot.drive.driveTrainByControls(Constants.stick.getRawAxis(1), Constants.stick.getRawAxis(0), 0.00, true);
            return true;
        }
    }
    ...
    public boolean distanceRobotFromTargetCorrectly(){
        ...
        if(distance < 30.0){
            Robot.drive.mecanumDrive.driveCartesian(0.1, 0.0, 0.0);
            return false;
        } else if(distance > 45.0){
            Robot.drive.mecanumDrive.driveCartesian(-0.1, 0.0, 0.0);
            return false;
        } else {
            Robot.drive.driveTrainByControls(0.0, Constants.stick.getRawAxis(0), Constants.stick.getRawAxis(2), true);
            return true;
        }
    }
    ...
    public void shoot() {
        ...
        if (shooting && !dumpShot && !lowDumpShot) {
            ...
            if (Math.abs(calculatedAngle - tempAngle) > 0.004) {
                ...
                double distance = getXDistanceToGoal(NetworkTableInstance.getDefault().getTable("limelight").getEntry("ty").getDouble(0.0));
                if(distance > 200.0){
    ...
    public void dumpShotLEDs() {
        double distance = getXDistanceFromFrontOfRobotToFender(NetworkTableInstance.getDefault().getTable("limelight").getEntry("ty").getDouble(0.0));

        if ((distance > 30.0) && (distance < 45.0)) {
    ...
    public void dumpShot() { // 92 inches according to distance method
        ...
        if (dumpShot && !shooting && !lowDumpShot) {
            ...
            if ((masterShooterMotor.getSelectedSensorVelocity(1) > thresholdVelocity) && setHoodYet) {
                if (DriverStation.isTeleop()) {
                    if (Constants.xbox.getRawAxis(3) > 0.5) {
                        Robot.intake.indexerMotor.set(-0.6);
                    } else {
                        Robot.intake.indexerMotor.set(0.0);
                    }
                } else {
    ...
    public void lowDumpShot() {
        ...
        if (lowDumpShot && !dumpShot && !shooting) {
